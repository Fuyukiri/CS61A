...
def quadratic(x, a, b, c):
    ...
    critical = -b / (2 * a)
    y1, y2, extreme = [f(x) for x in [lower_bound(x), upper_bound(x), critical]]
    if lower_bound(x) < critical < upper_bound(x):
        return interval(min(y1, y2, extreme), max(y1, y2, extreme))
    else:
